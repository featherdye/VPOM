...
public class GeofenceTrasitionService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if ( geoFenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER ||
            ...
            String geofenceTransitionstatus = getGeofenceTrasitionstatus(geoFenceTransition, triggeringGeofences );
            //String fenceID = triggeringGeofences.get(0).getRequestId();
            Log.d(TAG,"kaaaa"+geofenceTransitionDetails);
            lbcIntent.putExtra("fenceID",geofenceTransitionDetails);
            lbcIntent.putExtra("event",geofenceTransitionstatus);
            // Send notification details as a String
            sendNotification( geofenceTransitionDetails );
        }

//        if (geoFenceTransition==Geofence.GEOFENCE_TRANSITION_ENTER){
//            lbcIntent.putExtra("key",false);
//        }
//        else if(geoFenceTransition==Geofence.GEOFENCE_TRANSITION_EXIT){
//            lbcIntent.putExtra("key",true);
//        }
    }
    ...
    private String getGeofenceTrasitionstatus(int geoFenceTransition, List<Geofence> triggeringGeofences) {
        // get the ID of each geofence triggered
        ArrayList<String> triggeringGeofencesList = new ArrayList<>();
        for ( Geofence geofence : triggeringGeofences ) {
            triggeringGeofencesList.add( geofence.getRequestId() );
